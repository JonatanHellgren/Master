...
In the quantitative forecasting model by \autocite{Ajeya Cotra} and later summarized by \autocite{Robin Shah AN\#121}, they present a model that predict when TAI will be possible to be made based on a set of estimated parameters. The key idea is to estimate how much compute the human brain performs and then use the set of parameters to estimate how long it would take for computers to reach this amount of compute. When computers are on the same level as humans it is assumed that the creation of a TAI system will be possible to create. The set of parameters include how fast our hardware is developing, improvement in algorithms and how much an potential actor is willing to spend. The results where summarized as the following:
\begin{displayquote}
...
There are one thing worth mentioning when talking about the timelines for future TAI. It is not necessarily true that the amount of progress will continue to develop with the current rate. The field of AI has previously been through two 'winters' where the funding and excitement decreased. This was mainly due to high expectations that where not met. So if a third winter where to emerge we would expect the timeline to become a bit longer.
...
% Nott i smarter then us finns att h'mta
