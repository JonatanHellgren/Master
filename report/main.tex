...
What we can see to today is a large focus on developing AI, but not nearly as large focus on making sure that it will be safe. %\textbf{K"LLA}. 
...
Now that our environment is defined, we also need to add an impact measurement. Here we will use two impact measurements, one explicit pre-defined and another implicit one that is found automatically by utilizing the experience of another agent trained in the same environment. The pre-defined impact measurement is supposed to be a relatively simple one that is able to capture all environmental side effects. The purpose is to have something simple to compare our other less simple impact measurements. 

The explicit impact measurement will be defined as the number of foods present in the environment. This is similar to yielding the agent a negative reward when consuming the wrong type of food. However, while these types of impact measurents are only possible to define in an environment where all components can be captured and thus will not be a general solution, we can still utilize it to see if our implicit impact measurement is able to capture the same thing. Altough, when applying this in a partially observable environment issues arise, since the impact measurement is able to see the entire environment while the agent is only able to see parts of it. And, if we want to solve this by only counting the foods the agent can see then we would disourage the agent from performing actions which both removes food unnecessary and moves foods outside the agents vision. 

The method we will use to find the implicit impact measurement is to first train an agent using the PPO-algorithm, as described in the previous section. Then we will perform multiple rollouts using this pre-trained agent. From the rollouts we will save all the observations and the discounted future reward the agent got. Then we will train a third network to estimate the discounted reward given the observation. We will call this third network the \textit{manager}. The manager is a similar function as the critic, the difference is that the critic learns along side the actor while the manager learns from a pre-trained actor. 

The reason we split this up into two separate networks is that it will allow us to train the manager using augmentations. There are two reasons why we would like to use augmentations. Firstly, we can by rotating the grid find a more general function since only the distance to the food should matter and not direction to it. Secondly, we will also be able to generalize to other tasks which the agent has not been trained on, this can be done by switching the colors of everything in the environment, including the color the agent desires. 

With an trained manager we will define the value measurement to be the sum of the managers estimates for the colors that the agent does not desire. 

When we training the manager in a fully observable environment, the task is straightforward since the observation gives the full explaination to the discounted reward. However, when performing this in a partially observable environment we run into issues such as the agent does not know how many foods it has consumed previously and thus not how many there are left. Due to this issue, we will itilize a recurrent network that inputs all previous states and the current one to compute the estimate of the future discounted reward.
